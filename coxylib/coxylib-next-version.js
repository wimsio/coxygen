...
export async function walletEssentials(selectedWallet, Cip30Wallet, WalletHelper, Value, utxoAmount, j) {
    try {
        ...
        const wallet = await eval('window.cardano.' + selectedWallet);
        j.log({
            wallet
        });
        ...
        if (walletEnabled) {
            ...
            j.test("walletEssentials", "walletHandler", walletHandler).object()
...
export const showWalletData = async (walletData, utxoAmount, AssetClass, j) => {
    ...
    try {
        ...
        const collateralAda = String((await walletData.walletHelper.pickCollateral()).value.lovelace / BigInt(1000000));
        j.log({
            collateralAda
        });
        ...
        const tem = Object.values(utxos[0]);
        j.log({
            tem
        })
        const assets = Object.values(Object.values(utxos[0]))[0].value.assets.dump();
        j.log({
            assets
        })
        const assetsArray = Object.keys(assets).map((key, value) => [key, assets[key]]);
        j.log({
            assetsArray
        })
        const assetsAsStringArray = JSON.parse(JSON.stringify(assetsArray));
        j.log({
            assetsAsStringArray
        })
        ...
        const res = assetsAsStringArray.map((x) => {
            ...
            const assetsObjects = {
                tokenName: tokenName,
                assetQty: tokenQuantity,
                mph: mph,
                assetHexName: tokenHexName,
...
export const txFunc = async (walletData, hlib, tx, j, networkParamsUrl) => {
    ...
    const networkParams = new hlib.NetworkParams(await fetch(networkParamsUrl).then(response => response.json()));
    j.log({
        networkParams
    })
...
export const deadLine = (tx, networkParams, deadLineMinutes, j) => {
    ...
    after.setMinutes(now.getMinutes() + deadLineMinutes);
    j.log({
        after
    });
    tx.validFrom(before);
...
export const addSigner = (networkParams, partyAddress, partyAutxos, tx, signersPKHs[], owner = "", j) = {
    j.log({
        signersPKHs
    });
    ...
    await tx.finalize(networkParams, partyAddress, partyAutxos);j.log({
        tx
    });
    const partySignature = await partyAddress.signTx(tx);j.log({
        partySignature
    });
    tx.addSignatures(partySignature);j.log({
        tx
    });
    if (owner != "") {
        const ownerSignature = await owner.signTx(tx);
        j.log({
            ownerSignature
        });
        tx.addSignatures(ownerSignature);
        j.log({
            tx
        });
    } else {
...
export const submitTx = (tx, network, j) => {
...
export const attachScript = (tx, script, j) => {
...
export const addTxOutPuts = (tx, addresses[], datums[], j) => {
...
export const txInputs = (tx, utxos, j) => {
    tx.addInputs(utxos);
    j.log({
        tx
    })
...
export const mintTokens = (tx, mph, token, redeemer, j) => {
    tx.mintTokens(
        mph,
        token,
        redeemer
